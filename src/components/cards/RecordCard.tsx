...
const RecordCard = ({ record, onAccess }: { record: Record, onAccess: (id: string) => void }) => {
    ...
    return (
        <div className="bg-card rounded-xl p-5 shadow-sm cursor-pointer" onClick={handleClick}>
            ...
            <h2 className="font-semibold mb-1 overflow-hidden text-ellipsis whitespace-nowrap">{record.title}</h2 >
            <p className="text-xs text-muted-foreground mb-3 overflow-hidden text-ellipsis whitespace-nowrap">{record.category}</p>
            <p className="text-sm text-muted-foreground mb-4 overflow-hidden text-ellipsis whitespace-nowrap">{record.preview}</p>
