...
const ExamAISourceModal = ({
  ...
  const handleSubmit = () => {
    ...
    if (activeTab === "notes") {
      onSubmit({ source: "notes", noteIds: selectedNotes, questionCounts, language });
    } else {
      onSubmit({ source: "files", files: uploadedFiles, questionCounts, language });
    }
  };
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                max={10}
                value={essCount}
                onChange={(e) => setEssCount(Math.max(0, Number(e.target.value)))}
                disabled={isLoading}
              />
            </div>
          </div>
