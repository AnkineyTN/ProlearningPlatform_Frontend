...
function NoteEditorInner({
  ...
  const [editorInstance, setEditorInstance] = useState<BlockNoteEditorClass | null>(null);
    ...
    const handleMouseUp = () => {
      ...
      if (text.length > 0) {
        ...
        if (range && editorContainerRef.current) {
          ...
          setTooltipPos({ x: rect.left - editorRect.left, y: rect.top - editorRect.top - 40 });
...
function NoteEditorFallback({
  ...
  useEffect(() => { setEditorInstance(blockNoteEditor); }, [blockNoteEditor]);

  const handleEditorChange = useCallback(() => {
    if (editorInstance) onContentChange(JSON.stringify(editorInstance.document));
  }, [editorInstance, onContentChange]);

  useEffect(() => {
    const root = document.documentElement;
    const sync = () => setBlockNoteScheme(root.classList.contains("dark") ? "dark" : "light");
    sync();
    const observer = new MutationObserver(sync);
    observer.observe(root, { attributes: true, attributeFilter: ["class"] });
    return () => observer.disconnect();
  ...
  useImperativeHandle(innerRef, () => ({
    getHTML: async () => {
      if (editorInstance) return await editorInstance.blocksToHTMLLossy(editorInstance.document);
      return "";
    },
  }), [editorInstance]);
  ...
  const handleAISummarize = useCallback(async () => {
    if (!selectedText) return;
    if (!setId || !noteId) { toast.error("Invalid note"); return; }
    try {
      const response = await explainTextMutation.mutateAsync({
        setId,
        data: {
...
interface NoteEditorProps {
  ...
  onConnStatusChange?: (status: "connecting" | "connected" | "disconnected") => void;
  ...
  useEffect(() => {
    ...
    provider.awareness?.on("change", () => {
      ...
      provider?.awareness?.getStates().forEach((state) => {
        if (state?.user) {
          users.push({ name: String(state.user.name), color: String(state.user.color) });
        }
      });
