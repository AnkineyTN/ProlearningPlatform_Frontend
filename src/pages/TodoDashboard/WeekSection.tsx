...
const DayCard = ({
  ...
  const weekday = date.toLocaleDateString(i18n.language, { weekday: "short" }).toUpperCase();
  ...
  return (
    <button
      ...
      style={{
        background: selected ? "color-mix(in oklch, var(--pl-accent) 8%, var(--pl-bg-elev))" : "var(--pl-bg-elev)",
    ...
    >
      <div className='flex items-baseline justify-between'>
        <div>
          <div className='text-[9.5px] tracking-[0.18em] text-[var(--pl-text-faint)]'>{weekday}</div>
          <div
            className='text-[22px] font-[var(--font-display)] leading-none'
            style={{ color: isToday ? "var(--pl-accent-strong)" : "var(--pl-text)" }}
          >
            {dayNum}
          </div>
        </div>
        ...
        {previewTodos.map((td) => {
          ...
          return (
            <div
              ...
              style={{ color: isDone ? "var(--pl-text-faint)" : "var(--pl-text-muted)" }}
            >
              <span className='w-1 h-1 rounded-full flex-shrink-0' style={{ background: accent }} />
              <span className={`truncate ${isDone ? "line-through" : ""}`}>{td.title}</span>
            </div>
          );
        })}
        {hidden > 0 && (
    ...
    <div className='group flex items-center gap-3 rounded-[10px] px-3 py-2.5 hover:bg-[var(--pl-bg-hover)] transition-colors'>
      ...
      <button
        ...
        className={`w-[18px] h-[18px] rounded-full grid place-items-center flex-shrink-0 border ${
          isDone ? "bg-[var(--pl-accent)] border-[var(--pl-accent)]" : "bg-transparent border-[var(--pl-border-strong)]"
        }`}
      >
        {isDone && (
...
const WeekSection = ({
  ...
  const selectedLabel = new Date(selectedDate).toLocaleDateString(i18n.language, {
    weekday: "long",
    month: "short",
    day: "numeric",
  });
