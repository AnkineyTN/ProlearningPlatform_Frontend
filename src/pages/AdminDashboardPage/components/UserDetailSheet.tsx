...
import { X, FileText, Layers, ClipboardList, Timer } from 'lucide-react';
...
const StatCard = ({
  ...
  <div className='rounded-xl border border-border bg-[var(--pl-bg-elev)] p-4 flex items-center gap-3'>
    <div
      ...
      <div className='relative w-full max-w-sm bg-[var(--pl-bg)] border-l border-border h-full flex flex-col shadow-2xl'>
        <div className='flex items-center justify-between p-5 border-b border-border shrink-0'>
          <h2 className='font-semibold text-sm'>
            {t('adminDashboard.userDetailTitle')}
          </h2>
          <Button
            variant='ghost'
            size='icon'
            ...
            <div>
              ...
              <div className='flex gap-1.5 mt-1 flex-wrap'>
                ...
                {user.roles.map((r) => (
                  <span
                    key={r}
                    className='text-[10px] px-2 py-0.5 rounded-full border border-border bg-[var(--pl-bg-sunken)] text-muted-foreground'
                  >
                    {r}
                  </span>
                ))}
          ...
          <div className='space-y-2 text-sm'>
            ...
            <div className='space-y-1.5'>
              {[
                ...
                {
                  label: t('adminDashboard.colLanguage'),
                  value: user.language ?? '—',
                },
                {
                ...
                },
                {
                  ...
                  value: user.hearAppFrom ?? '—',
                },
                {
                  label: t('adminDashboard.detailRegistered'),
                  value: formatDate(stats?.registeredAt),
                },
              ].map(({ label, value }) => (
