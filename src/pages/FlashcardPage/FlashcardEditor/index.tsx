...
export default function FlashcardEditor({
  ...
  const { data: flashcardData, isLoading } = useFlashcardDetail(setId, flashcardId || 0);

  const { title: locationTitle, description: locationDescription, privacy, generatedFlashcards } =
    location.state || {};

  const [title, setTitle] = useState(locationTitle || "");
  const [description, setDescription] = useState(locationDescription || "");
  const [cards, setCards] = useState<FlashcardCard[]>([
    { id: crypto.randomUUID(), term: "", definition: "", imageUrl: "", assetId: undefined, _action: "CREATE" },
  ]);
    ...
    setCards((prev) =>
      prev
        .map((card) => {
          ...
          if (typeof id === "number") return { ...card, _action: "DELETE" as const };
          return null as any;
        })
        .filter(Boolean),
    );
  ...
  const handleSave = async () => {
    ...
    try {
      if (isUpdateMode) {
        const toCreate = cards.filter((c) => c._action === "CREATE" && typeof c.id === "string");
        const toUpdate = cards.filter((c) => c._action === "UPDATE" && typeof c.id === "number");
        const toDelete = cards.filter((c) => c._action === "DELETE" && typeof c.id === "number").map((c) => c.id as number);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-10 bg-card border-b border-border">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between gap-6">
          ...
          <Button onClick={handleSave} disabled={isSaving} className="px-6 flex-shrink-0">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-card border border-border rounded-xl p-6 mb-8 space-y-5">
          <div>
            <Label htmlFor="fc-title" className="text-xs uppercase tracking-widest text-muted-foreground/60 mb-2 block">
            ...
            <Input
              ...
              className="text-lg font-medium bg-background border-border focus:border-primary"
            />
          </div>
          <div>
            <Label htmlFor="fc-desc" className="text-xs uppercase tracking-widest text-muted-foreground/60 mb-2 block">
              Description
            </Label>
            <Textarea
              id="fc-desc"
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 bg-card border border-border rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2 text-muted-foreground">
            <button className="w-8 h-8 rounded-lg border border-border bg-card flex items-center justify-center hover:bg-secondary transition-colors cursor-pointer">
        ...
        <button
          onClick={addCard}
          className="w-full flex items-center justify-center gap-2 py-5 rounded-xl border-2 border-dashed border-border text-sm text-muted-foreground hover:border-primary/50 hover:text-primary hover:bg-card transition-colors cursor-pointer mt-2"
        >
